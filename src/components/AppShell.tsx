...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    { to: "/mapa", label: "Mapa", icon: ClipboardList },
    { to: "/biblioteca", label: "Biblioteca", icon: BookMarked },
    { to: "/conteudo-programatico", label: "Conteúdo", icon: FileText },
    { to: "/plan", label: "Plano", icon: ListChecks },
    { to: "/study", label: "Estudo", icon: BookOpen },
    { to: "/review", label: "Revisões", icon: CalendarRange },
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-2 font-semibold tracking-tight">
            ...
            <span>
              PSCPP <span className="text-muted-foreground font-normal">Command Center</span>
            </span>
          </Link>
          <nav className="hidden md:flex items-center gap-1">
            {links.map((l) => (
              <Link
                ...
                activeProps={{
                  className: "px-3 py-1.5 rounded-md text-sm bg-accent text-foreground",
                }}
          ...
          {links.map((l) => {
            ...
            return (
              <Link
                ...
                activeProps={{
                  className:
                    "flex-1 flex flex-col items-center gap-0.5 py-2 text-[11px] text-primary",
                }}
                activeOptions={{ exact: l.to === "/" }}
              >
